...
void LCD_Change_Brash_Color(u16 color){
	POINT_COLOR = color;
}
...
int flag;
extern void Update_Choice(int x,int y);
extern void Chess_Update();
extern int isSelect,isComputer,isBoard;
extern int exist();
extern int chess_table[5][5];
...
static void getPos(u16 xScreen, u16 yScreen) {
	...
	int dist = 0;
	isReady = 0;
	for(i = 10, dist = 9999; i <= QPSTARTX + QPSIZEX; i += 22) {
		if(abs(i-x) <= dist) {
			xScreen = i;
			dist = abs(i-x);
		} 
	}
	for(j = 10, dist = 9999; j <= QPSTARTY + QPSIZEY; j += 40) {
		if(abs(j-y) <= dist) {
			yScreen = j;
			dist = abs(j-y);
		}
	...
	LCD_ShowNum(280,2,xScreen,4,12);
	LCD_ShowNum(280,14,yScreen,4,12);
	if(yScreen == 290){
		ax = 0;
		flagy = 1;
	}
	...
	else if(yScreen == 210 || yScreen == 170){
		ax = 2;
		flagy = 1;
	}
	else if(yScreen == 130){
	...
	else if(xScreen == 164 || xScreen == 142){
		by = 2;
		flagx= 1;
	}
	else if(xScreen == 76 || xScreen == 98){
		by = 3;
		flagx = 1;
	}
	else if(xScreen == 32 || xScreen == 54){
		by = 4;
		flagx = 1;
	}
	if(xScreen == 186 && (yScreen == 50 || yScreen == 10) && exist()){
		isSelect = !isSelect;
	}
	if (xScreen == 142 && yScreen == 50 && isComputer == 0){
		isBoard = !isBoard;
		if(isBoard){
			LCD_Change_Brash_Color(BLACK);
			LCD_ShowString(285,100,3*16,16,16,(uint8_t *)" ON");
		}
		else {
			LCD_Change_Brash_Color(BLACK);
	...
	if ((xScreen == 76 || xScreen == 98) && yScreen == 50 && isBoard == 0){
		isComputer = !isComputer;
		if(isComputer){
			LCD_Change_Brash_Color(BLACK);
			LCD_ShowString(285,170,3*16,16,16,(uint8_t *)" ON");
		}
		else {
			isWaiting = 0;
			LCD_Change_Brash_Color(BLACK);
